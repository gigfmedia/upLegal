...
serve(async (req) => {
  ...
  try {
    const { email, lawyerName, lawyerId } = await req.json()

    if (!email || !lawyerName) {
      throw new Error('Email y nombre del abogado son requeridos')
    }
    ...
    const { error: tokenError } = await supabaseClient
      .from('review_tokens')
      .insert({
        appointment_id: 'manual-' + Date.now(), // ID temporal para envío manual
        token: reviewToken,
    ...
    return new Response(
      JSON.stringify({ 
        ...
        email,
        reviewUrl,
        message: 'Email de reseña enviado correctamente'
      }),
      { headers: { ...corsHeaders } }
    )
