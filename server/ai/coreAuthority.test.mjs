...
function harness({paid=true, linked=true, count=3}={}) {
 ...
 const supabase={rpc,storage:{from:()=>({download})},from(table){
 ...
 const ctx=vm.createContext({console:quiet,z,Buffer,supabase,hasCanonicalDocumentReference,resolveAnalysisModel,buildChatContext,buildChatSystemPrompt,buildChatUserPrompt,CHAT_LIMITS,getProCaseHeader,verifyDocumentClaims,buildAnalysisSystemPrompt,buildAnalysisUserPrompt,
 app:{get:(p,h)=>routes[`get ${p}`]=h,post:(p,h)=>routes[`post ${p}`]=h},getUserIdFromToken:async()=>tokenUser,
 ...
 async function call(name,body={},params={}){const [method,path]=paths[name];const res={statusCode:200,status(n){this.statusCode=n;return this;},json(b){this.body=b;return this;}};await routes[`${method} ${path}`]({headers:{authorization:'Bearer fixture'},params:{caseId:name==='provision'?caseId:workspaceId,id:id(10),...params},body},res);return res;}
  return {rows,provider,download,rpc,call,ctx,supabase,asForeign:()=>{tokenUser=foreign;}};
}
